               ...
               <div className="flex-1 overflow-y-auto p-5 no-scrollbar pb-10">
                   {detailTab === 'identity' && (
                       <div className="space-y-6 animate-fade-in">
                           ...
                           {/* 多模型群聊：该角色独立聊天模型后端（可选） */}
                           <div className="bg-amber-50/60 rounded-3xl p-4 shadow-sm border border-amber-200/60 space-y-2">
                               <div className="flex items-center justify-between">
                                   <label className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">独立聊天模型（可选）</label>
                                   {formData.chatApiConfig ? (
                                       <button onClick={() => handleChange('chatApiConfig', undefined)} className="text-[10px] bg-white text-amber-700 px-2 py-1 rounded font-bold hover:bg-amber-100">清除，用全局模型</button>
                                   ) : null}
                               </div>
                               <p className="text-[11px] text-slate-500">留空则群聊里该角色使用「设置」中的全局模型。填了就各自用自家密钥（例如这个角色接 DeepSeek，另一个角色接智谱 GLM，两人在群里各自用自家的模型互聊）。</p>
                               <div className="grid grid-cols-1 gap-2">
                                   <input
                                       value={formData.chatApiConfig?.baseUrl || ''}
                                       onChange={(e) => handleChange('chatApiConfig', { ...(formData.chatApiConfig || { model: '', apiKey: '' }), baseUrl: e.target.value })}
                                   ...
                                   />
                                   <input
                                       value={formData.chatApiConfig?.apiKey || ''}
                                       onChange={(e) => handleChange('chatApiConfig', { ...(formData.chatApiConfig || { baseUrl: '', model: '' }), apiKey: e.target.value })}
                                       className="w-full bg-white rounded-2xl px-3 py-2 text-xs border border-slate-200"
                                       placeholder="API Key"
                                   />
                                   <input
                                       value={formData.chatApiConfig?.model || ''}
                                       onChange={(e) => handleChange('chatApiConfig', { ...(formData.chatApiConfig || { baseUrl: '', apiKey: '' }), model: e.target.value })}
                                       className="w-full bg-white rounded-2xl px-3 py-2 text-xs border border-slate-200"
                                       placeholder="模型名（如 deepseek-chat / glm-4）"
                                   />
                               </div>
                           </div>
