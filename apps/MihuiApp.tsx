...
const fieldClass = 'w-full rounded-2xl border border-[#e7d4db] bg-white/90 px-4 py-3 text-sm text-[#3b3035] outline-none transition focus:border-[#b98191] focus:ring-4 focus:ring-[#eadce1] placeholder:text-[#b9a8ae]';
...
const MihuiApp: React.FC = () => {
    ...
    const renderHome = () => (
        <div className="flex-1 overflow-y-auto px-5 pb-10">
            <section className="mt-4 rounded-[2rem] bg-gradient-to-br from-[#171316] via-[#33252b] to-[#a87686] p-6 text-white shadow-[0_22px_45px_-22px_rgba(67,40,50,.72)] relative overflow-hidden">
                <div className="absolute -right-8 -top-10 w-40 h-40 rounded-full bg-white/15" />
                <div className="absolute right-10 bottom-0 w-24 h-24 rounded-full bg-[#e8cfd7]/20" />
                ...
                <div className="mt-6 flex gap-3 relative">
                    <button onClick={() => { setMatchError(''); setScreen('match'); }} className="flex-1 rounded-2xl bg-[#f7edf0] px-4 py-3 text-sm font-black text-[#6f4553] shadow-lg active:scale-[.98] transition">选择偏好</button>
                    <button onClick={() => match(true)} disabled={matching} className="flex-1 rounded-2xl bg-black/80 px-4 py-3 text-sm font-black text-[#f1dce3] active:scale-[.98] disabled:opacity-60 transition flex items-center justify-center gap-2">
            ...
            <div className="mt-7 flex items-center justify-between">
                ...
                <span className="rounded-full bg-[#f1e3e8] px-3 py-1 text-xs font-bold text-[#8c5366]">{state.sessions.length} 人</span>
            </div>
    ...
    const renderMatch = () => (
        <div className="flex-1 overflow-y-auto px-5 pb-10">
            <div className="mt-3">
                <p className="text-[11px] font-bold tracking-[.28em] text-[#a96d80]">MATCH FILTER</p>
            ...
            <div className="mt-6 space-y-5">
                <section>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                        {(Object.keys(genderLabel) as MihuiGender[]).map(key => (
                            <button key={key} onClick={() => patchPreferences('gender', key)} className={`${chipClass} ${draftPrefs.gender === key ? 'bg-[#8c5366] text-white shadow-md' : 'bg-white text-slate-500 border border-[#e7d4db]'}`}>{genderLabel[key]}</button>
                        ))}
                    </div>
                ...
                {([
                    ...
                    <section key={key}>
                        <label className="text-xs font-black text-slate-600 flex items-center gap-1.5"><Icon size={15} className="text-[#a96d80]" />{label}</label>
            ...
            <button onClick={() => match(false)} disabled={matching} className="mt-7 w-full rounded-2xl bg-[#292126] py-4 text-sm font-black text-[#f4e5ea] shadow-[0_16px_30px_-15px_rgba(54,35,43,.75)] disabled:opacity-60 active:scale-[.99] transition">
        ...
        return (
            <div className="absolute inset-0 z-50 bg-black/55 backdrop-blur-md grid place-items-center p-6">
                <div className="w-full rounded-[2rem] bg-white p-6 text-center shadow-2xl">
                    <div className="mx-auto w-14 h-14 rounded-2xl bg-[#ead8de] text-[#8c5366] grid place-items-center"><Heart size={30} weight="fill" /></div>
                    ...
                    <button onClick={addToNeuralLink} className="mt-6 w-full rounded-2xl bg-[#292126] py-3.5 text-sm font-black text-[#f4e5ea] flex items-center justify-center gap-2"><UserPlus size={18} />加入神经链接</button>
            ...
            <>
                <div className="px-4 pb-3 border-b border-[#e7d4db] bg-white/80 backdrop-blur-xl shrink-0">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                        <span className="text-[10px] font-black text-[#8c5366]">{stage}</span>
                        <div className="h-1.5 flex-1 rounded-full bg-[#f1e3e8] overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-[#6d4957] to-[#c08b9b] transition-all duration-700" style={{ width: `${activeSession.affinity}%` }} /></div>
    ...
    return (
        <div className="h-full w-full flex flex-col bg-[#f6f0f2] text-slate-800 animate-fade-in relative overflow-hidden">
