...
const UserApp: React.FC = () => {
    ...
    const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            addToast(err?.message || '头像处理失败', 'error');
        } finally {
            e.target.value = '';
        }
    };
    ...
    return (
        <div className="h-full w-full bg-slate-50 flex flex-col animate-fade-in">
            <div
                className="bg-white/70 backdrop-blur-md border-b border-slate-100 shrink-0 sticky top-0 z-10"
                style={{ paddingTop: 'var(--safe-top)' }}
            >
                <div className="flex items-center px-4 py-3 gap-2">
                    <button onClick={closeApp} className="p-2 -ml-2 rounded-full hover:bg-black/5 active:scale-90 transition-transform" aria-label="返回">
                ...
                <div className="flex gap-1.5 px-4 pb-2.5">
                    {([['profile', '我的档案'], ['life', '生活记录']] as const).map(([key, label]) => (
                        <button
                            key={key}
                            onClick={() => setTab(key)}
                            className={`px-4 py-1.5 rounded-full text-xs font-bold transition-colors ${tab === key ? 'bg-primary text-white shadow-sm' : 'bg-slate-100 text-slate-400'}`}
                        >
                            {label}
                        </button>
                    ))}
                ...
                {tab === 'profile' && <>
                    <div className="bg-white rounded-[1.75rem] shadow-[0_10px_30px_-12px_rgba(80,70,120,0.25)] border border-slate-100 overflow-hidden">
                        <div className="relative h-24" style={{ background: 'linear-gradient(135deg, hsl(var(--primary-hue),var(--primary-sat),72%) 0%, hsl(var(--primary-hue),var(--primary-sat),60%) 100%)' }}>
                            <div className="absolute -top-6 -right-4 w-28 h-28 rounded-full bg-white/20" />
                        ...
                        <div className="px-6 pb-6 -mt-12">
                            <button
                                ...
                                className="relative w-24 h-24 rounded-full group mx-auto block"
                                aria-label="更换整体头像"
                            >
                                <span className="block w-full h-full rounded-full ring-4 ring-white bg-slate-100 overflow-hidden shadow-md">
                                    <img src={userProfile.avatar} className="w-full h-full object-cover group-hover:opacity-80 transition-opacity" alt="用户头像" />
                                </span>
                                <span className="absolute bottom-0.5 right-0.5 w-7 h-7 rounded-full bg-primary text-white flex items-center justify-center ring-2 ring-white shadow-sm">
                            ...
                            <p className="mt-2 text-center text-[10px] text-slate-400">整体头像用于群聊和其他场合；私聊可在下方单独设置。</p>
                    ...
                    <div className="bg-white rounded-[1.75rem] shadow-[0_10px_30px_-12px_rgba(80,70,120,0.18)] border border-slate-100 p-5">
                        ...
                        <p className="text-[11px] text-slate-400 mb-3 leading-relaxed">这些内容会发送给 AI，帮助角色了解你的身份、性格和偏好。</p>
                        <textarea
