...
const fallback = (entry: StoryTheaterEntry): StoryTheaterImageConfig => ({
    ...
    imageCount: entry.imageGeneration?.imageCount === 1 ? 1 : 2,
        ...
        >
            ...
            >
                ...
                <div className='min-h-0 flex-1 overflow-y-auto overscroll-contain border-y border-slate-200 px-5'>
                    <div className='py-4'><div className='flex items-center justify-between gap-4'><div><div className='text-sm font-semibold'>每轮自动配图</div><p className='mt-1 text-[10px] leading-5 text-slate-500'>正文先显示，再按世界书格式提取本轮的两幅剧情画面。</p></div><Toggle value={draft.enabled} onChange={enabled => setDraft(current => ({ ...current, enabled }))} /></div></div>
                    ...
                    <label className='block border-t border-slate-200 py-4'><span className='text-[10px] font-bold text-slate-500'>每轮图片数量</span><select value={draft.imageCount === 1 ? '1' : '2'} onChange={event => setDraft(current => ({ ...current, imageCount: event.target.value === '1' ? 1 : 2 }))} className='mt-1.5 w-full rounded-2xl border border-slate-200 bg-white px-3 py-3 text-xs outline-none'><option value='2'>两张 · 从本轮正文提取两幅画面（默认）</option><option value='1'>一张 · 只保留第一幅剧情画面</option></select></label>
