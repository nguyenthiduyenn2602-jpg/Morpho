...
const FarawayApp: React.FC = () => {
    ...
    return (
        <div className="relative h-full w-full overflow-hidden bg-[#171310] text-white" style={{ paddingTop: 'var(--chrome-top, var(--safe-top))' }}>
            ...
            <div className="absolute left-[82px] top-[calc(var(--chrome-top,0px)+7px)] z-10 whitespace-nowrap text-[10px] tracking-[.34em] text-white/65">MORPHO · ELSEWHERE</div>
            {displayChar && <div className="absolute left-[76px] top-[calc(var(--chrome-top,0px)+32px)] z-10 inline-flex min-w-[136px] max-w-[205px] items-center gap-3 rounded-2xl border border-white/15 bg-black/45 px-3 py-2 backdrop-blur"><img src={displayChar.avatar} className="h-9 w-9 shrink-0 rounded-full object-cover" /><div className="min-w-0"><b className="block truncate text-sm">{displayChar.name}</b><div className="truncate text-[10px] text-white/65">{activeJourney ? `外出中 · ${activeJourney.destination}` : '房间安静着'}</div></div></div>}
