...
import { buildMealPlannerPrompt, extractJsonObject, formatMealCompletionForChat, formatMealPlanForChat, normalizeMealPlan } from './mealPlanner';

describe('mealPlanner', () => {
    it('把家常与平价限制写进导演提示词', () => {
        const prompt = buildMealPlannerPrompt([
            { id: 'egg', name: '鸡蛋', quantity: 3, unit: '个', category: '肉蛋' },
        ], { targetKcal: 1600, diners: 1, mealsPerDay: 2, pushCharacterIds: [], goal: '控热量', dislikes: '不吃香菜', kitchenNote: '只有炒锅' }, '2026-09-07');
    ...
    it('按偏好只保留指定餐数，并可整理成私聊文本', () => {
        ...
        expect(formatMealPlanForChat(plan, '沈欢')).toContain('【沈欢今日饮食小票');
        expect(formatMealPlanForChat(plan, '沈欢')).toContain('第一餐：鸡蛋面');
        expect(formatMealPlanForChat(plan, '沈欢')).toContain('顺手补买：豆腐一块');
        expect(formatMealPlanForChat(plan, '沈欢')).not.toContain('两顿也好好吃');
        expect(formatMealCompletionForChat(plan, '第一餐', '沈欢')).toContain('沈欢这顿已经好好吃完啦');
    });
