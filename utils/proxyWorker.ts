 ...
 *
 * 默认指向 Morpho 自己部署的实例；也可以在「设置 → 网络代理 (Worker)」中
 * 填写另一套自托管地址，以上全部能力会自动切换，无需改代码。
 *
 * 注意：网易云音乐（MusicContext）和小红书 Lite 各自在自己的 App / 设置里有
 * 独立的 worker 地址输入框，走各自的持久化，不受这里影响。
 */

export const DEFAULT_PROXY_WORKER = 'https://morpho-proxy.nguyenthiduyenn2602.workers.dev';
...
//   - sullymeow.ccwu.cc：原版当前公共实例；Morpho 不再共用作者后端
const STALE_HOSTS = [
...
// Cloudflare AMSG Worker 没有 localStorage；后台工具调用通过这里注入用户自己的代理。
