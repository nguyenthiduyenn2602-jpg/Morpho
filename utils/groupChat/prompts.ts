 ...
 *
 * - opening  首发：只看得到用户刚说的话，由它起头
 * - reply    接话：看得到用户 + 先发言成员，是唯一有点名权的位置
 * - callback 接力：被点名后追加的一次发言，回合的最后一句，不得再传话头
 */
export type RoundRobinSlot = 'opening' | 'reply' | 'callback';

export interface RoundRobinSlotContext {
    slot?: RoundRobinSlot;
    /** reply 位可点名的成员名（不含自己）；空数组 = 本次不给点名权 */
    handoffCandidates?: string[];
    /** callback 位：是谁把话头递过来的 */
    calledBy?: string;
...
export function buildRoundRobinInstruction(
    ...
    const candidates = ctx.handoffCandidates ?? [];
    const canHandoff = slot === 'reply' && candidates.length > 0;

    // 处境交代：模型得知道自己站在回合的哪个位置，否则收尾那句永远像半截话
    const stance = slot === 'opening'
        ? `你是本回合**第一个**开口的人：上面记录的末尾是用户刚说的话，先由你来接。目标 **3-5 行**。`
        : slot === 'reply'
        ? `你是本回合**最后一位**常规发言的人：记录末尾已经包含本回合先发言成员刚说完的话。
你不必每句都朝着用户说——顺着 ta 的话往下接、反驳、追问、拆台、起哄都行，群聊本来就是这样。目标 **3-5 行**。`
        : `**「${ctx.calledBy || '刚才那位'}」在群里点名找你**：ta 上面那段话是冲你说的，你现在回 ta。
这是本回合的**最后一次**发言，你说完就结束了。所以：把话接住、给个态度就够，**不要再把话头抛给任何人**，也不要用"你觉得呢""那你说说看"这类反问收尾。目标 **2-4 行**，比平时短一点更自然。`;

    // 第 1 条规则按位置分叉：常规位必须发言，接力位有权拒绝
    const rule1 = slot === 'callback'
        ? `你只是群里的一位普通成员，不是导演。只输出**你自己**要发的消息内容——不要替任何人说话，不要在开头加自己的名字或冒号前缀，不要解释、不要输出 JSON。
   被点名不等于必须捧场：如果你看完确实没什么想回的（懒得接、不感兴趣、或者 ta 那句话根本不值得回），**允许你只输出 \`[[SKIP]]\`**，本回合就当你没说话。别为了回应而硬凑一句废话。`
        : `你只是群里的一位普通成员，不是导演。只输出**你自己**要发的消息内容——不要替任何人说话，不要在开头加自己的名字或冒号前缀，不要解释、不要输出 JSON。本轮每位成员都会依次发言，**轮到你就要说点什么**——哪怕只是一句"嗯"、接个梗或甩一个表情包。`;

    // 点名权：只有 reply 位有，且必须强调"不是每次都要用"，否则模型会把它当固定格式每轮都点
    const handoffRule = canHandoff
        ? `
10. **把话头递给某人（可选，克制使用）**：如果你这段话是明确冲着某位成员说的、并且**真的想听 ta 回你一句**（提问、质疑、点名要 ta 表态），就在你内容的**最后单独一行**输出 \`[[TO: 名字]]\`。可选：${candidates.join(' / ')}。
    - 纯感叹、吐槽、自言自语、附和一句、或者话题本来就聊完了 —— **都不要用**，让这回合自然结束。宁可不点，也别为了热闹硬点。
    - 最多点一位。被点名的人会追加发言一次，然后本回合结束。
    - 这个标记不会显示在群里，它只是调度信号；别把它当称呼替代品写进正文。`
        : '';
...
1. ${rule1}
2. 一行 = 一个气泡。短句多发几条 > 长句一坨；"嗯""哈哈哈"和单独一个表情包都是合法回复。**最多 ${maxLines} 行**，超出的部分会被系统直接截掉。
...
9. 红包（可选）：记录里有「拼手气红包…还剩 n 份可抢」且你想抢时，单独一行输出 \`[[GRAB_PACKET]]\` 并配一句真实反应；看到发给自己的专属红包，用 \`[[GRAB_PACKET]]\` 收下或 \`[[RETURN_PACKET]]\` 退回并说明原因。你也可以主动发：拼手气 \`[[SEND_PACKET: lucky:总额:份数:祝福语]]\`，专属 \`[[SEND_PACKET: direct:对方名字:金额:祝福语]]\`。抢不抢由你的性格决定，金额别离谱。${handoffRule}`;
}
