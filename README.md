...
**① 主代理 Worker**（Morpho 发布版使用自己的实例，源码单文件 [`worker/index.js`](./worker/index.js)）
覆盖：联网搜索 / 热榜（Brave）、WebDAV 云备份、GitHub 云备份、Notion、飞书多维表格、麦当劳 / 瑞幸点单 MCP、网页抓取、Fish Audio TTS、音乐生成、网易云音乐（默认）。
👉 其他二改仍应在 **「设置 → 网络代理 (Worker)」** 填上自己部署的地址，以上能力可一键切走。（`wrangler deploy` 把 `worker/index.js` 部署到自己的 CF 账号，拿到地址填进去即可。）
...
Morpho 已移除依赖原作者共享后端的「彼方」及社区招募链路，不会向原作者服务器发送这类数据。
