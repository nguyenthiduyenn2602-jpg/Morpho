...
const StoryImageSettingsButton: React.FC<Props> = ({ entry, onChange }) => {
    ...
    return <>
        ...
        {open && <div className='fixed inset-0 z-[95] flex items-end justify-center bg-slate-900/35' onClick={() => setOpen(false)}>
            <div className='story-safe-sheet max-h-[90dvh] w-full max-w-sm overflow-y-auto rounded-t-[28px] bg-stone-100 px-5 pt-5 shadow-2xl' onClick={event => event.stopPropagation()}>
                <div className='flex items-start gap-4'><div className='min-w-0 flex-1'><div className='text-[9px] font-bold uppercase tracking-[.22em] text-violet-500'>Story illustration</div><h2 className='mt-1 text-lg font-semibold'>本剧情自动配图</h2><p className='mt-1 text-[10px] leading-5 text-slate-500'>接口、Key、模型、采样器、Steps 与 CFG 统一读取全局生图 2.0。</p></div><button onClick={() => setOpen(false)} className='grid h-10 w-10 shrink-0 place-items-center rounded-full border border-slate-200 bg-white'><X size={17} /></button></div>
                <div className='mt-5 border-y border-slate-200 py-4'><div className='flex items-center justify-between gap-4'><div><div className='text-sm font-semibold'>每轮生成一张</div><p className='mt-1 text-[10px] text-slate-500'>正文先显示，随后结合本轮与近期上下文画图。</p></div><Toggle value={draft.enabled} onChange={enabled => setDraft(current => ({ ...current, enabled }))} /></div></div>
                {!globalReady && <div className='mt-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-[10px] leading-5 text-amber-700'>全局生图 2.0 尚未完整配置。可以先填写本页，开启前需到私聊生图 2.0 设置 URL、Key 与模型。</div>}
                <label className='mt-5 block'><span className='text-[10px] font-bold text-slate-500'>本剧情画师串 / 风格</span><textarea value={draft.styleTags || ''} onChange={event => setDraft(current => ({ ...current, styleTags: event.target.value }))} placeholder='artist:name, cinematic lighting, detailed background' className='mt-1.5 min-h-28 w-full resize-y rounded-2xl border border-slate-200 bg-white p-3 text-xs leading-5 outline-none' /></label>
                <label className='mt-4 block'><span className='text-[10px] font-bold text-slate-500'>配图画幅</span><select value={`${draft.width}x${draft.height}`} onChange={event => { const [width, height] = event.target.value.split('x').map(Number); setDraft(current => ({ ...current, width, height })); }} className='mt-1.5 w-full rounded-2xl border border-slate-200 bg-white px-3 py-3 text-xs outline-none'><option value='1216x832'>横图 1216×832（默认）</option><option value='1344x768'>宽横图 1344×768</option><option value='1024x1024'>方图 1024×1024</option><option value='832x1216'>竖图 832×1216</option><option value='768x1344'>长竖图 768×1344</option></select></label>
                <p className='mt-4 text-[10px] leading-5 text-slate-400'>人物锚点在“剧情设置 → 让谁参与”下方调整；多人场景会按本轮真正出镜的人物组合标签。</p>
                <button onClick={() => void save()} className='mb-2 mt-5 h-12 w-full rounded-2xl bg-slate-900 text-xs font-bold text-white'>保存配图设置</button>
            </div>
        </div>}
    </>;
};
